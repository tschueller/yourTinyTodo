    ...
    parseAnchor: function()
    {
        ...
        for(var i=0; i<a.length; i++)
        {
            ...
            switch(s) {
                ...
                case "task": if(a[++i].match(/^-?\d+$/)) { p[s] = a[i]; if (p.list == null) p.list = '-1';} break;
                case "alltasks": p.list = '-1'; break;
            }
        }

        if(p.list) this.options.openList = p.list;
        if(p.task) this.options.openTask = p.task;
...
// Added pagereload timeout; tsc, 31.12.2011
...
function loadTasks(opts)
{
    ...
    _mtt.db.request('loadTasks', {
        ...
        if (_mtt.options.openTask) {
            editTask(_mtt.options.openTask);
            _mtt.options.openTask = 0;
        }
    });
};
...
{
    // make URLs clickable
    s = s.replace(/(^|\s|>)(www\.([\w\!\(\){}AäÖöÜüß#$%&~\/.\-\+;:=,\?\[\]@]+?))(,|\.|:|)?(?=\s|&quot;|&lt;|&gt;|\"|<|>|$)/gi, '$1<a href="http://$2" target="_blank">$2</a>$4');
    return s.replace(/(^|\s|>)((?:http|https|ftp):\/\/([\w\!\(\){}AäÖöÜüß#$%&~\/.\-\+;:=,\?\[\]@]+?))(,|\.|:|)?(?=\s|&quot;|&lt;|&gt;|\"|<|>|$)/ig, '$1<a href="$2" target="_blank">$2</a>$4');
};
...
function editTask(id)
{
    ...
    toggleEditAllTags(1);
    showEditForm();
    $('#editTaskId').attr('href', "#task/"+item.id).text("#"+item.id);
    return false;
};
